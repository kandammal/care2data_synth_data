...
TERMINOLOGY PROVENANCE:
  All MedDRA codes in this module are from a curated synthetic library, NOT
  from the licensed MedDRA dictionary (which requires an MSSO subscription).
  Hierarchy levels (PT -> HLT -> HLGT -> SOC) follow MedDRA structural
  conventions, and numeric codes are plausible but synthetic. Do not use
  these codes for regulatory submission or pharmacovigilance without
  cross-referencing against a licensed MedDRA release.
