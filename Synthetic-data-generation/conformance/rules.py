"""
SDTM Conformance Rules — Data Layer

Provides rule primitives (check functions), data classes (ValidationResult,
RuleSeverity), and domain metadata (REQUIRED_VARIABLES, KEY_VARIABLES,
DATE_VARIABLES) used by the repair layer (layer.py).

For tagged-category validation, see v2_validator.py.
"""
...
def check_dm_arm_consistency(dm_df: pd.DataFrame, all_datasets: Dict[str, pd.DataFrame]) -> ValidationResult:
    ...
    for idx, row in dm_df.iterrows():
        # Screen failures have blank ARMCD/ARM with ARMNRS='SCREEN FAILURE'
        if row.get('ARMNRS') == 'SCREEN FAILURE':
            if row.get('ARMCD') not in ['', None] or row.get('ACTARMCD') not in ['', None]:
