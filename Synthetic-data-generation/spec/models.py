...
class RealismConfig(BaseModel):
    """Configuration for realism enhancements."""
    # Visit window jitter
    enable_visit_jitter: bool = True

    # Weibull dropout timing (shape < 1 = front-loaded)
    enable_weibull_dropout: bool = True
    dropout_weibull_shape: float = Field(default=0.7, gt=0, le=2.0)

    # Cross-domain physiological conditioning
    enable_baseline_conditioning: bool = True

    # AE → Lab/VS feedback perturbations
    enable_ae_feedback: bool = True

    # AE → CM pharmacological mapping (instead of random CM selection)
    enable_pharmacological_cm: bool = True

    # MH → CM background medication linkage
    enable_mh_background_cm: bool = True

    # AR(1) autocorrelation for lab/VS within-subject dynamics
    enable_ar1_dynamics: bool = True
    ar1_rho_vs: float = Field(default=0.45, ge=0, le=0.95)
    ar1_rho_lb: float = Field(default=0.50, ge=0, le=0.95)
    vs_noise_scale: float = Field(default=0.35, gt=0, le=1.0,
        description="Fraction of baseline_sd used as visit-to-visit noise")
    lb_noise_scale: float = Field(default=0.25, gt=0, le=1.0)

    # Missing data model
    enable_missingness: bool = True
    visit_miss_rate: float = Field(default=0.07, ge=0, le=0.30)
    vs_assessment_miss_rate: float = Field(default=0.02, ge=0, le=0.20)
    lb_assessment_miss_rate: float = Field(default=0.04, ge=0, le=0.20)

    # Partial dates for MH
    enable_partial_dates: bool = True
    mh_partial_date_rate: float = Field(default=0.15, ge=0, le=0.50)

    # EX dose modifications from AEs
    enable_ex_dose_modifications: bool = True

    # Lab normal range indicators
    enable_lab_normal_range: bool = True

    # Treatment arm–specific AE rate modifiers (placebo gets lower rate)
    enable_arm_ae_modifiers: bool = True
    placebo_ae_rate_modifier: float = Field(default=0.6, gt=0, le=2.0)
