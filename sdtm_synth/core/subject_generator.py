...
def generate_canonical_subject(
    ...
    if is_screen_failure:
        ...
        screen_fail_days = rng.draw_int("timeline", "screen_fail_days", low=1, high=28)
    ...
    # Concomitant Medications (derived from AEs + standalone)
    subject.conmeds = _generate_conmeds(rng, spec, accepted_aes, first_dose_date, actual_end, subj_canonical_id)
    ...
    # Vital Signs (record-keyed RNG, per-period)
    subject.vs_records = _generate_vital_signs(rng, spec, treatment_sequence, first_dose_date, actual_end, subj_id=subj_canonical_id)
    
    # Laboratory (record-keyed RNG, per-period)
    subject.lb_records = _generate_labs(rng, spec, treatment_sequence, first_dose_date, actual_end, subject.latent_traits, subj_id=subj_canonical_id)
...
def _accept_ae_candidates(
    ...
    target_count = spec.ae_model.ae_rate_multiplier * total_exposure_months * frailty_mult
...
def _derive_disposition(
    ...
    if dropout_u < adjusted_dropout_rate:
        # Subject drops out — clamp dropout to active treatment period
        ...
        if dropout_date < first_dose_date:
            dropout_date = first_dose_date + timedelta(days=1)
...
def _generate_conmeds(
    ...
    """Generate CM events: AE-linked + standalone."""
    prefix = f"{subj_id}/" if subj_id else ""
    events = []
    ordinal = 0
    ...
    # Standalone CMs
    n_standalone = rng.draw_int("CM", RNGComponent.CM_COUNT, low=0, high=3)
    cm_lib = spec.cm_model.cm_library if spec.cm_model.cm_library else DEFAULT_CM_LIBRARY
...
def _generate_exposures(
    ...
    for period in treatment_sequence:
        ...
        for reg_idx, reg in enumerate(arm_spec.regimen):
            ...
            # adherence_propensity [0,1] → fraction of planned period kept [0.4, 1.0]
            if rng is not None and adherence < 0.95:
                planned_duration = (ex_end - ex_start).days
                if planned_duration > 1:
                    # Low adherence → early stop. Map adherence [0,1] → keep_frac [0.4, 1.0]
                    keep_frac = 0.4 + 0.6 * adherence
                    # Add per-period/regimen jitter via RNG (unique key per regimen)
                    jitter_u = rng.draw("EX", period.period, reg_idx, "adherence_jitter")[0]
                    # Jitter ±20% of keep_frac
...
def _generate_vital_signs(
    ...
    """Generate VS records keyed by (period, visitnum, testcd).
    
    Record-keyed RNG ensures adding a test or visit doesn't change existing values.
    """
    prefix = f"{subj_id}/" if subj_id else ""
    records = []
    ...
    vs_visits = _get_measurement_visits(spec, treatment_sequence, first_dose_date, actual_end)
    
    # Generate baseline values first (keyed per-test)
    baselines = {}
    for test_item in vs_tests:
        bl_val = rng.draw_normal("VS", "baseline", test_item.testcd,
                                 mean=test_item.baseline_mean, std=test_item.baseline_sd)
        bl_val = max(test_item.plausible_min, min(test_item.plausible_max, bl_val))
        baselines[test_item.testcd] = bl_val
    
    for visit_info in vs_visits:
        ...
        for test_item in vs_tests:
            ...
            # Record-keyed draw: draw("VS", period, visitnum, testcd, component)
            noise = rng.draw_normal("VS", period, visitnum, tc, RNGComponent.VALUE,
                                    mean=0.0, std=test_item.baseline_sd * 0.15)
            
            # Trend: slight drift over time
            days_on_study = (visit_date - first_dose_date).days if visit_date >= first_dose_date else 0
            trend = test_item.trend_slope * days_on_study / 30.0
            
            value = baselines[tc] + noise + trend
...
def _generate_labs(
    ...
    """Generate LB records keyed by (period, visitnum, testcd)."""
    prefix = f"{subj_id}/" if subj_id else ""
    records = []
    lb_tests = spec.lb_model.tests
    
    lb_visits = _get_measurement_visits(spec, treatment_sequence, first_dose_date, actual_end)
    ...
    for visit_info in lb_visits:
        ...
        for test_item in lb_tests:
            ...
            noise = rng.draw_normal("LB", period, visitnum, tc, RNGComponent.VALUE,
                                    mean=0.0, std=test_item.baseline_sd * 0.12)
            
            days_on_study = (visit_date - first_dose_date).days if visit_date >= first_dose_date else 0
            trend = test_item.trend_slope * trend_mult * days_on_study / 30.0
...
def _get_measurement_visits(
    ...
    if spec.visits:
        ...
        for v in spec.visits:
            visit_date = first_dose_date + timedelta(days=v.nominal_day - 1)
            if visit_date > actual_end:
                continue
            
            # Determine which period this visit falls in
            period = 1
            study_day_offset = v.nominal_day
