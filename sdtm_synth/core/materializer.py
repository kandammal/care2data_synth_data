...
def materialize_all(
    ...
    for subj in sorted(subjects, key=lambda s: s.identity.subject_index):
        usubjid = f"{study_id}-{subj.identity.siteid}-{subj.identity.subjid}"
        first_dose = _get_first_dose_date(subj)
        rfstdtc = first_dose
        subj_prefix = f"{subj.identity.canonical_id}/"  # P0: subject-scope DS canonical IDs
        
        # ── DM ──
        dm_records.append(_materialize_dm(subj, study_id, usubjid, first_dose))
        
        if subj.is_screen_failure:
            # Screen failures get minimal DS record
            ds_rec = _make_ds_record(study_id, usubjid, 1, "SCREEN FAILURE", "SCREEN FAILURE",
                                      "PROTOCOL MILESTONE", "", subj.disposition.date, rfstdtc)
            ds_records.append(ds_rec)
            provenance.register(f"{subj_prefix}DS_PROTOCOL_MILESTONE_SCREENFAIL", "DS", usubjid, 1)
            continue
        
        # ── AE ──
        # Sort by onset date for SEQ assignment (Contract 8)
        ...
        if subj.disposition.outcome == "COMPLETED":
            ...
            # Create AE↔DS death relationship
        ...
        # ── VS ── (from canonical truth, Phase 2)
        ...
        # ── LB ── (from canonical truth, Phase 2)
    ...
    # Build RELREC
    datasets['RELREC'] = provenance.rebuild_relrec(relationships, study_id)
...
def _materialize_dm(subj: CanonicalSubject, study_id: str, usubjid: str, first_dose: Optional[date]) -> dict:
    ...
    rec = {
        ...
        'RFENDTC': format_iso_date(subj.end_date) if subj.end_date else '',
        'RFXSTDTC': '' if is_sf else (format_iso_date(first_dose) if first_dose else ''),
        'RFXENDTC': '',
        'RFICDTC': format_iso_date(subj.timeline.consent_date),
        'RFPENDTC': format_iso_date(subj.end_date) if subj.end_date else '',
...
def _materialize_mh(mh, study_id, usubjid, seq, rfstdtc) -> dict:
    return {
        ...
        'MHBODSYS': '',
        'MHSTDTC': format_iso_date(mh.start_date) if mh.start_date else '',
...
def _materialize_sv(subj, spec, study_id, usubjid, first_dose, rfstdtc) -> list:
    """Generate SV records from spec visits."""
    ...
    for seq, visit in enumerate(spec.visits, 1):
        visit_date = first_dose + timedelta(days=visit.nominal_day - 1)
...
def _materialize_lb(lb: LBRecord, study_id: str, usubjid: str, seq: int,
                     rfstdtc, subj: CanonicalSubject) -> dict:
    """Render LB row from canonical truth."""
